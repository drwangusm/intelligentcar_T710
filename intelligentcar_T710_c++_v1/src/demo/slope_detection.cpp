...
#include "../src/detection/slope_detection.cpp" //坡道AI检测与路径规划类
...
int main(int argc, char *argv[])
{
    ...
    SlopeDetection slopeDetection;                  // 坡道检测
    ...
    detection = Detection::DetectionInstance("../res/samples/slope.mp4", "../res/model/yolov3_mobilenet_v1"); // Video输入源
    ...
    while (1)
    {
        ...
        // [04] 坡道检测与路径规划
        slopeDetection.slopeDetection(trackRecognition, resultAI->predictor_results);
        slopeDetection.drawImage(trackRecognition, imageCorrect);

        icarShow.setNewWindow(3, imageCorrect, "imgSlope"); // 添加需要显示的图像
