/**
 * @file busy_detection.cpp
 * @author Leo ()
 * @brief 施工区检测与路径规划Demo
 * @version 0.1
 * @date 2022-03-30
 *
...
#include "../src/detection/busy_detection.cpp" //施工区AI检测与路径规划类
...
int main(int argc, char *argv[])
{
    ...
    detection = Detection::DetectionInstance("../res/samples/busyArea.mp4", "../res/model/yolov3_mobilenet_v1"); // Video输入源
    ...
    while (1)
    {
        ...
        // [04] 施工区检测与路径规划
        busyareaDetection.busyareaDetection(trackRecognition, resultAI->predictor_results);
        busyareaDetection.drawImage(trackRecognition, imageCorrect);

        icarShow.setNewWindow(3, imageCorrect, "imgBusyarea"); // 添加需要显示的图像
