...
public:
    ...
    bool gasStationDetection(TrackRecognition &track, vector<PredictResult> predict)
    {
        ...
        {
        case GasStationStep::GasStationNone: //[01] 加油站标志检测
            for (int i = 0; i < predict.size(); i++)
            {
                if (predict[i].label == LABEL_GASSTATION) // 加油站标志检测
                {
                    counterRec++;
                    break;
                }
            ...
            if (counterRec)
            {
                ...
                {
                    gasStationStep = GasStationStep::GasStationEnable; // 加油站使能
                    counterRec = 0;
                    counterSession = 0;
                    exitTwoEnable = false;
                }
        ...
        case GasStationStep::GasStationEnable: //[02] 加油站使能
        {
            vector<POINT> gas = searchGasstation(predict); // 搜索加油站标志
            if (gas.size() <= 0)                           // 离开加油站标志后|开始入站搜索
            {
                vector<POINT> pointsCone = searchCones(predict);
                _pointNearCone = searchNearestCone(track.pointsEdgeLeft, pointsCone); // 搜索右下锥桶
                if (_pointNearCone.x > ROWSIMAGE * 0.15)                              // 当车辆开始靠近右边锥桶：准备入库
                {
                    counterRec++;
                    if (counterRec >= 2)
                    {
                        gasStationStep = GasStationStep::GasStationEnter; // 进站使能
                        counterRec = 0;
                        counterSession = 0;
                    }
                }
                ...
                // 出口检测
                POINT exitChanel = searchWriteTwo(predict);
                if (exitChanel.x > 0)
                {
                    exitTwoEnable = true;
                    _pointTwo = exitChanel;
                }
                if (exitChanel.x > ROWSIMAGE * 0.45)
                {
                    gasStationStep = GasStationStep::GasStationEnter; // 进站使能
                    counterRec = 0;
                    counterSession = 0;
        ...
        case GasStationStep::GasStationEnter: //[03] 进站使能
        {
            ...
            else // 第二阶段：检查右下锥桶坐标满足巡航条件
            {
                ...
                if ((coneRightDown.x > ROWSIMAGE / 3 && coneRightDown.y > COLSIMAGE - 80) || counterSession > 20)
                {
                    ...
                    if (counterRec >= 2)
                    {
                        gasStationStep = GasStationStep::GasStationCruise; // 巡航使能
                        counterRec = 0;
                        counterSession = 0;
                    }
                }
        ...
        case GasStationStep::GasStationCruise: //[04] 巡航使能
        {
            vector<POINT> pointsCone = searchCones(predict);      // 计算锥桶的坐标
            ...
            if (track.pointsEdgeLeft.size() > ROWSIMAGE / 6 && track.pointsEdgeRight.size() > ROWSIMAGE / 6)
            {
                ...
                if (counterRec >= 2)
                {
                    gasStationStep = GasStationStep::GasStationExit; // 出站使能
                    counterRec = 0;
                    counterSession = 0;
                }
            }
            ...
            POINT exitChanel = searchBlueTwo(predict); // 到达二号出口
            _pointTwo = exitChanel;
            if (exitChanel.x > ROWSIMAGE * 0.45)
            {
                gasStationStep = GasStationStep::GasStationExit; // 出站使能
        ...
        case GasStationStep::GasStationExit: //[05] 出站使能
        {
            vector<POINT> pointsCone = searchCones(predict);  // 计算锥桶的坐标
            ...
            {
                gasStationStep = GasStationNone; // 出站结束
                counterRec = 0;
                counterSession = 0;
            }
            else
        ...
        if (gasStationStep == GasStationNone) // 返回加油站控制模式标志
            return false;
        else
            return true;
    }
     ...
     *
     */
    void drawImage(TrackRecognition track, Mat &imageGasStation)
    {
        for (int i = 0; i < pointEdgeDet.size(); i++)
        {
            circle(imageGasStation, Point(pointEdgeDet[i].y, pointEdgeDet[i].x), 2, Scalar(92, 92, 205), -1); // 锥桶坐标：红色
        }
        ...
        if (_pointTwo.x > 0)
            circle(imageGasStation, Point(_pointTwo.y, _pointTwo.x), 3, Scalar(200, 200, 200), -1);
    }
...
private:
    ...
    enum GasStationStep
    {
        GasStationNone = 0, // 未触发
        GasStationEnable,   // 加油站操作使能（标志识别成功）
        GasStationEnter,    // 加油站进站
        GasStationCruise,   // 加油站巡航
        GasStationExit      // 加油站出站
    };
    ...
    /**
     * @brief 从AI检测结果中检索加油站坐标集合
     *
     * @param predict AI检测结果
     * @return vector<POINT>
     */
    vector<POINT> searchGasstation(vector<PredictResult> predict)
    {
        vector<POINT> gas;
        for (int i = 0; i < predict.size(); i++)
        {
            if (predict[i].label == LABEL_GASSTATION) // 检测
            {
                gas.push_back(POINT(predict[i].y + predict[i].height / 2, predict[i].x + predict[i].width / 2));
            }
        }
     ...
     * @return vector<POINT>
     */
    POINT searchWriteTwo(vector<PredictResult> predict)
    {
        for (int i = 0; i < predict.size(); i++)
        {
            if (predict[i].label == LABEL_WRITETWO) // 数字2
            {
     ...
     * @return vector<POINT>
     */
    POINT searchBlueTwo(vector<PredictResult> predict)
    {
        for (int i = 0; i < predict.size(); i++)
        {
            if (predict[i].label == LABEL_BLUETWO) // 数字2
            {
                return POINT(predict[i].y + predict[i].height / 2, predict[i].x + predict[i].width / 2);
            }
        }
    ...
    {
        POINT point(0, 0);
        double disMin = 30; // 右边缘锥桶离赛道左边缘最小距离
