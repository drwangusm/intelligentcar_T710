...
public:
    ...
    bool ringRecognition(TrackRecognition &track, Mat &imagePath)
    {
        ...
        {
            if (track.widthBlock[i].y < track.widthBlock[i - 1].y && track.widthBlock[i].y > COLSIMAGE * 0.4 && track.widthBlock[i].x > 30 &&
                ((track.stdevLeft < 50 && track.stdevRight > 100) || ringStep == RingStep::Entering)) //搜索突然变宽的路径行数
            {
                ++countWide;
            }
            else
            ...
            // [1] 入环判断
            if ((ringStep == RingStep::None || ringStep == RingStep::Entering) && countWide >= 5)
            {
                ...
                //内圆检测
                if (ringTypeTemp == RingType::RingRight && track.pointsEdgeRight[i].y - colEnterRing >= 3)
                {
                    _tmp_ttttt = 2;
                    ringEnable = true;
            ...
            if (ringEnable == true && ringStep == RingStep::Entering)
            {
                ...
                {
                    if (track.pointsEdgeRight[i].y >= COLSIMAGE - 2 && i != track.widthBlock.size() - 1)
                    {
        ...
        {
            //本场没判出环，且没有分叉
            if (!track.spurroad.empty() && rowRepairLine < track.pointsEdgeLeft.size() - 1 && rowBreakpointLeft > ROWSIMAGE / 2)
            {
            ...
            {
                if (ringType == RingType::RingRight && track.pointsEdgeLeft.size() > 1)
                {
                    _tmp_ttttt = 4;
                    ...
                    POINT startPoint(ROWSIMAGE - 10, 1);  //补线：起点
        ...
        if (ringStep == RingStep::Inside)
        {
            if (ringType == RingType::RingRight)
            {
                ...
                for (int i = 0; i < track.pointsEdgeLeft.size(); i += 3)
                {
                    ...
                    if (i > rowBreakLeft && track.pointsEdgeLeft[i].y - track.pointsEdgeLeft[rowBreakLeft].y < 5)
                    {
                        break; //寻找到出环口：出环补线
                    }
                }
                ...
                if (!track.pointsEdgeLeft.empty() && track.pointsEdgeLeft[rowBreakLeft].y > COLSIMAGE * 0.75)
                {
                ...
                else if (track.pointsEdgeLeft.size() - rowBreakLeft > 20)
                {
                    ...
                    {
                        rowBreakpointRight = track.pointsEdgeLeft[track.validRowsLeft].x;
                        POINT p_end(rowBreakpointRight, COLSIMAGE - 1); //补线终点为左边有效行顶点
                        POINT p_mid((track.pointsEdgeLeft[rowBreakLeft].x + rowBreakpointRight) * 3 / 8, track.pointsEdgeLeft[rowBreakLeft].y / 2);
                ...
                else if (track.pointsEdgeLeft.size() - rowBreakLeft <= 20)
                {
                    ...
                    POINT p_start(max(rowBreakpointRight, ROWSIMAGE - 80), 0);
    ...
    void drawImage(TrackRecognition track, Mat &ringImage)
    {
        ...
        putText(ringImage, to_string(track.validRowsRight) + " | " + to_string(track.stdevRight),
                Point(COLSIMAGE - 100, ROWSIMAGE - 50), FONT_HERSHEY_TRIPLEX, 0.3, Scalar(0, 0, 255), 1, CV_AA);
