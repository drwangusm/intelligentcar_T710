...
int main(int argc, char const *argv[])
{
    ...
    if (motionController.params.debug)
    {
        ...
        printAiEnable = true;
        icarShow.init(4); // 图像显示窗口初始化                                                                              // 开启AI绘制                                                                                   // AI检测结果绘制
    }
    else
    {
        cout << "等待发车!!!" << endl;
    ...
    while (1)
    {
        ...
        if (motionController.params.debug)
        {
            Mat imageTrack = imageCorrect.clone();            // RGB
        ...
        if (motionController.params.FarmlandEnable) // 赛道元素是否使能
        {
            ...
            {
                if (farmlandDetection.farmlandDetection(trackRecognition, resultAI->predictor_results))
                {
                    if (roadType == RoadType::BaseHandle) // 初次识别-蜂鸣器提醒
                        driver->buzzerSound(1);           // OK
        ...
        if (motionController.params.DepotEnable) // 赛道元素是否使能
        {
            ...
            {
                if (depotDetection.depotDetection(trackRecognition, resultAI->predictor_results))
                {
                    if (roadType == RoadType::BaseHandle) // 初次识别-蜂鸣器提醒
                        driver->buzzerSound(1);           // OK
                    ...
                    if (motionController.params.debug)
                    {
                        ...
                        icarShow.setNewWindow(2, imageDepot, "imgDepot"); // 添加需要显示的图像
                        imshowRec = true;
        ...
        if (true) // 赛道元素是否使能
        {
            ...
            {
                if (granaryDetection.granaryDetection(trackRecognition, resultAI->predictor_results))
                {
                    if (roadType == RoadType::BaseHandle) // 初次识别-蜂鸣器提醒
                        driver->buzzerSound(1);           // OK
        ...
        if (motionController.params.BridgeEnable) // 赛道元素是否使能
        {
            ...
            {
                if (bridgeDetection.bridgeDetection(trackRecognition, resultAI->predictor_results))
                {
                    if (roadType == RoadType::BaseHandle) // 初次识别-蜂鸣器提醒
                        driver->buzzerSound(1);           // OK
        ...
        if (motionController.params.SlowzoneEnable) // 赛道元素是否使能
        {
            ...
            {
                if (slowZoneDetection.slowZoneDetection(trackRecognition, resultAI->predictor_results))
                {
                    if (roadType == RoadType::BaseHandle) // 初次识别-蜂鸣器提醒
                        driver->buzzerSound(1);           // OK
        ...
        // [13] 控制中心计算
        if (trackRecognition.pointsEdgeLeft.size() < 30 && trackRecognition.pointsEdgeRight.size() < 30 && roadType != RoadType::BridgeHandle && roadType != RoadType::GranaryHandle && roadType != RoadType::DepotHandle && roadType != RoadType::FarmlandHandle) // 防止车辆冲出赛道
        {
            counterOutTrackA++;
            counterOutTrackB = 0;
        ...
        if (counterRunBegin > 30) ////智能车启动延时：前几场图像不稳定
        {
            ...
            case RoadType::BridgeHandle:
                motionController.motorSpeed = motionController.params.speedBridge; // 匀速控制
                break;
            case RoadType::SlowzoneHandle:
                motionController.motorSpeed = motionController.params.speedSlowzone; // 匀速控制
                break;
            ...
            if (!motionController.params.debug) // 调试模式下不控制车辆运动
            {
                if (roadType == RoadType::DepotHandle)
                {
                    if (depotDetection.depotStep == 4)
                        driver->carControl(0, motionController.servoPwm); // 串口通信，姿态与速度控制
        ...
        if (motionController.params.debug)
        {
            ...
            case RoadType::RingHandle:                                                                                            // 环岛赛道处理                                                                                    // 环岛赛道处理
                putText(imageCorrect, "[2] Ring", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::CrossHandle:                                                                                            // 十字道路处理                                                                                       // 十字道路处理
                putText(imageCorrect, "[3] Cross", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::FreezoneHandle:                                                                                            // 泛行区处理                                                                                          // 泛行区处理
                putText(imageCorrect, "[4] Freezone", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::GarageHandle:                                                                                            // 车库处理                                                                                          // 车库处理
                putText(imageCorrect, "[5] Garage", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::GranaryHandle:                                                                                            // 粮仓处理                                                                                              // 加油站处理
                putText(imageCorrect, "[6] Granary", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::DepotHandle:                                                                                            // 修车厂处理                                                                                   // 施工区处理
                putText(imageCorrect, "[7] Depot", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::BridgeHandle:                                                                                            // 坡道(桥)处理                                                                                 // 坡道处理
                putText(imageCorrect, "[8] Bridge", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::SlowzoneHandle:                                                                                            // 慢行区（动物出没）处理                                                                                   // 坡道处理
                putText(imageCorrect, "[9] Slowzone", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            case RoadType::FarmlandHandle:                                                                                             // 农田区域处理                                                                                     // 坡道处理
                putText(imageCorrect, "[10] Farmland", Point(10, 20), cv::FONT_HERSHEY_TRIPLEX, 0.3, cv::Scalar(0, 255, 0), 1, CV_AA); // 显示赛道识别类型
                break;
            }
