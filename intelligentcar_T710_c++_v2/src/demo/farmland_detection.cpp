...
#include "../src/detection/farmland_detection.cpp" //农田区域AI检测与路径规划类
...
int main(int argc, char *argv[])
{
    ...
    FarmlandDetection farmlandDetection;            // 农田区域检测类
    ...
    detection = Detection::DetectionInstance("../res/samples/farmland.mp4", "../res/model/yolov3_mobilenet_v1"); // Video输入源
    ...
    while (1)
    {
        ...
        // [04] 农田区域检测与路径规划
        farmlandDetection.farmlandDetection(trackRecognition, resultAI->predictor_results);
        farmlandDetection.drawImage(trackRecognition, imageCorrect);
        icarShow.setNewWindow(2, imageCorrect, "imgFarmland"); // 添加需要显示的图像
