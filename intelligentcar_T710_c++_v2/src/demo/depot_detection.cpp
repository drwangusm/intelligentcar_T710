...
#include "../src/detection/depot_detection.cpp" //维修厂AI检测与路径规划类
...
int main(int argc, char *argv[])
{
    ...
    DepotDetection depotDetection;                  // 维修厂检测类
    ...
    detection = Detection::DetectionInstance("../res/samples/depot.mp4", "../res/model/yolov3_mobilenet_v1"); // Video输入源
    ...
    while (1)
    {
        ...
        // [04] 维修厂检测与路径规划
        depotDetection.depotDetection(trackRecognition, resultAI->predictor_results);
        depotDetection.drawImage(trackRecognition, imageCorrect);
        icarShow.setNewWindow(2, imageCorrect, "imgDepot"); // 添加需要显示的图像
