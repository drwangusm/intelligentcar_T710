...
#include "../src/detection/bridge_detection.cpp" //坡道（桥）AI检测与路径规划类
...
int main(int argc, char *argv[])
{
    ...
    BridgeDetection bridgeDetection;                // 坡道（桥）检测类
    ...
    detection = Detection::DetectionInstance("../res/samples/bridge.mp4", "../res/model/yolov3_mobilenet_v1"); // Video输入源
    ...
    while (1)
    {
        ...
        trackRecognition.drawImage(imageTrack);           // 赛道线识别结果绘制到图像
        icarShow.setNewWindow(2, imageTrack, "imgTrack"); // 添加需要显示的图像

        // [04] 坡道（桥）检测与路径规划
        bridgeDetection.bridgeDetection(trackRecognition, resultAI->predictor_results);
        bridgeDetection.drawImage(trackRecognition, imageCorrect);

        icarShow.setNewWindow(3, imageCorrect, "imgBridge"); // 添加需要显示的图像
