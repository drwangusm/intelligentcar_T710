...
#include "../src/detection/slowzone_detection.cpp" //慢行区AI检测与路径规划类
...
int main(int argc, char *argv[])
{
    ...
    SlowZoneDetection slowzoneDetection;            // 减速慢行检测类
    ...
    detection = Detection::DetectionInstance("../res/samples/slowzone.mp4", "../res/model/yolov3_mobilenet_v1"); // Video输入源
    ...
    while (1)
    {
        ...
        // [04] 慢行区检测与路径规划
        slowzoneDetection.slowZoneDetection(trackRecognition, resultAI->predictor_results);
        slowzoneDetection.drawImage(trackRecognition, imageCorrect);

        icarShow.setNewWindow(3, imageCorrect, "imgSlowzone"); // 添加需要显示的图像
