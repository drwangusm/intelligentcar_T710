...
int main(int argc, char *argv[])
{
    ...
    VideoCapture capture("../res/samples/crossroad.mp4"); // 打开本地图像
    if (!capture.isOpened())
    {
    ...
    while (1)
    {
        ...
        crossroadRecognition.crossroadRecognition(trackRecognition, predict);
        crossroadRecognition.drawImage(trackRecognition, imageCorrect);

        icarShow.setNewWindow(3, imageCorrect, "imgCross"); // 添加需要显示的图像
