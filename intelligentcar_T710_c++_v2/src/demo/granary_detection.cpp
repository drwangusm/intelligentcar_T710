...
#include "../src/detection/granary_detection.cpp" //粮仓区域AI检测与路径规划类
...
int main(int argc, char *argv[])
{
    ...
    GranaryDetection granaryDetection;              // 粮仓区域检测类
    ...
    detection = Detection::DetectionInstance("../res/samples/granary.mp4", "../res/model/yolov3_mobilenet_v1"); // Video输入源
    ...
    while (1)
    {
        ...
        // [04] 粮仓区域检测与路径规划
        granaryDetection.granaryDetection(trackRecognition, resultAI->predictor_results);
        granaryDetection.drawImage(trackRecognition, imageCorrect);
        icarShow.setNewWindow(2, imageCorrect, "imgGranary"); // 添加需要显示的图像
